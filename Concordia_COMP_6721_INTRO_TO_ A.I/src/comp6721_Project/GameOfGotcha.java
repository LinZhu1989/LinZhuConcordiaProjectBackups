...
public class GameOfGotcha {
	...
	public GameBoard aiMove(GameOfGotcha player, int level, int deepth) {
		...
		switch (level) {
		case 0:
			return moveMethod.MinMax_BestMove_NoAlpha(player);
		case 1:
	...
	public static void showMenu2() {
		...
		System.out
				.println(">> 0.MinMax_BestMove\t\t\t(MinMax Search 3-4 Deepth(without alpha-beta pruning))");
		System.out
		...
		if (gameMode == 2) {
			...
			while (true) {
				...
				if (choices1.size() == 0) {
					...
					long time1 = System.currentTimeMillis();
					P1.updateBoard(P1.aiMove(P1, 0, 0));
					System.out
							.println("\nComputer Player 1 has moved successfully!\n");

					P1.printBoard();
					long time2 = System.currentTimeMillis();
					System.out.println("\n\nThinking time of Player 1 is: "
							+ (time2 - time1) + " milliseconds");
				}
				...
				if (choices2.size() == 0) {
					...
					long time1 = System.currentTimeMillis();
					P2.updateBoard(P2.aiMove(P2, 1, 0));
					System.out
							.println("\nComputer Player 2 has moved successfully!\n");
					P2.printBoard();
					long time2 = System.currentTimeMillis();
					System.out.println("\n\nThinking time of Player 2 is: "
							+ (time2 - time1) + " milliseconds");
				}
