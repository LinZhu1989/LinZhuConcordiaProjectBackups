...
public class AIHeuristic {
	...
	public GameBoard Manual_BestMove(GameOfGotcha player) {

		int plyDepth = checkPieces(player.getBoard().board());
		if(plyDepth==2){
			return myMinMaxL2Move(player);
		}else{
	...
	public GameBoard MinMax_BestMove(GameOfGotcha player) {
		...
		for (int i = 0; i < size; i++) {
			int score = alphabet(child.get(i), plyDepth - 1, -Integer.MAX_VALUE, Integer.MAX_VALUE, false);
	...
	public GameBoard myNagaMaxL2Move(GameOfGotcha player) {
		...
		for (int i = 0; i < size1; i++) {
			...
			for (int j = 0; j < size2; j++) {
				int l2Score = -lvl2Child.get(j).getNewHeuristic("P2");
		...
		for (int i = 0; i < size1; i++) {
			...
			for (int j = 0; j < size2; j++) {
				...
				for (int m = 0; m < size3; m++) {
					int l3Score = -lvl3Child.get(m).getNewHeuristic("P1");
			...
			for (int j = 0; j < size2; j++) {
				...
				for (int m = 0; m < size3; m++) {
					...
					for (int n = 0; n < size4; n++) {
						int l4Score = -lvl4Child.get(n).getNewHeuristic("P2");
	...
	public GameBoard myMinMaxL3Move(GameOfGotcha player) {
		...
		for (int i = 0; i < size1; i++) {
			...
			for (int j = 0; j < size2; j++) {
				...
				size3 = lvl3Child.size();
				for (int m = 0; m < size3; m++) {
					int l3Score = lvl3Child.get(m).getNewHeuristic("P2");
